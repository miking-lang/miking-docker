...
Using the `debian12.10` baseline for x86_64 as an example, we build it by
running:

```sh
make build-baseline PLATFORM=linux/amd64 BASELINE=debian12.10
```

This will create the `mikinglang/baseline:<ver>-debian12.10-linux-amd64` image.
...
```sh
make build-miking PLATFORM=linux/amd64 BASELINE=debian12.10
```

This will create the versioned image
`mikinglang/miking:<miver>-debian12.10-linux-amd64`.
...
```sh
make build-miking-dppl PLATFORM=linux/amd64 BASELINE=debian12.10
```

This will create the versioned image
`mikinglang/miking-dppl:<dpplver>-debian12.10-linux-amd64`.
...
Sticking with the example of `debian12.10` for `linux/amd64`, push the build
images to Docker Hub by running:

```sh
make push-baseline PLATFORM=linux/amd64 BASELINE=debian12.10
make push-miking   PLATFORM=linux/amd64 BASELINE=debian12.10
```
